...
static iree_status_t iree_hal_task_command_buffer_push_descriptor_set(
    ...
    iree_host_size_t binding_offset, iree_host_size_t binding_count,
    const iree_hal_descriptor_set_binding_t* bindings) {
  iree_hal_task_command_buffer_t* command_buffer =
      (iree_hal_task_command_buffer_t*)base_command_buffer;

  if (IREE_UNLIKELY(set < 0) ||
      IREE_UNLIKELY(set >= IREE_HAL_LOCAL_MAX_DESCRIPTOR_SET_COUNT)) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "set %zu out of bounds", set);
  } else if (IREE_UNLIKELY(binding_offset + binding_count >=
                           IREE_HAL_LOCAL_MAX_DESCRIPTOR_BINDING_COUNT)) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "binding count overflow at %zu + %zu (%zu)",
  ...
  iree_host_size_t binding_base =
      set * IREE_HAL_LOCAL_MAX_DESCRIPTOR_BINDING_COUNT + binding_offset;
  for (iree_host_size_t i = 0; i < binding_count; ++i) {
    // TODO(benvanik): track mapping so we can properly map/unmap/flush/etc.
    iree_hal_buffer_mapping_t buffer_mapping;
    IREE_RETURN_IF_ERROR(iree_hal_buffer_map_range(
        bindings[i].buffer,
        local_set_layout->bindings[binding_offset + i].access,
        bindings[i].offset, bindings[i].length, &buffer_mapping));
    command_buffer->state.bindings[binding_base + i] =
        buffer_mapping.contents.data;
    command_buffer->state.binding_lengths[binding_base + i] =
